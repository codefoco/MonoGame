...
namespace Microsoft.Xna.Framework.Graphics 
{
	...
	public sealed class SpriteFont 
    {
        ...
        public unsafe void MeasureString(string text, out Vector2 size, int maxLength, float preferredWidth)
        {
            ...
            if (count > 1)
            {
                kerning = GetKerning(text[0], text[1]);
            }

            Vector2 offset = new Vector2(-kerning + 1, lineSpacing);
            float width = 0f;
            ...
            fixed (Glyph* pGlyphs = Glyphs)
            {
                for (int i = 0; i < count; i++)
                {
                    ...
                    int index = GetGlyphIndexOrDefault(c);
                    ...
                    kerning = GetKerning(prev_char, c);
